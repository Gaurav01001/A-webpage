...
// Preload textures or establish a pattern
// Since pages array has full paths now, we can preload directly
pages.forEach((page) => {
    if (page.front) useTexture.preload(page.front);
    if (page.back) useTexture.preload(page.back);
});
...
const Page = ({
    ...
    const picture = useTexture(front, (t) => {
        t.colorSpace = SRGBColorSpace;
    });

    const picture2 = useTexture(back, (t) => {
        t.colorSpace = SRGBColorSpace;
    });
    ...
    const manualSkinnedMesh = useMemo(() => {
        ...
        mesh.bind(skeleton);
        return mesh;
    }, [picture, picture2]); // Re-create if textures change (though they shouldn't for same page)
